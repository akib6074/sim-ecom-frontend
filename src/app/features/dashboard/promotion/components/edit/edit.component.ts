...
export class EditComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    if (this.token.isAdmin()) {
      this.router.navigate(['/dashboard/promotion/edit']);
    } else {
  ...
  onShopChange(data: any) {
    ...
    if (selectedShop && selectedShop.length > 0) {
      this.editPromotionForm.controls.shopTypeID.setValue(
        selectedShop[0].typeId
      );
    }
  }
  ...
  onProductChange(data: any) {
    ...
    if (selectedProduct && selectedProduct.length > 0) {
      this.editPromotionForm.controls.categoryID.setValue(
  ...
  saveData() {
    this.isLoading = true;
    const promotion = this.editPromotionForm.value;
    promotion.type = parseInt(promotion.type, 10);
    this.promotionService
      .update(this.id, this.editPromotionForm.value)
      ...
      this.promotionService
        ...
        .subscribe(
          (res) => {
            ...
            this.editPromotionForm.controls.promotionCoverImage.setValue(
              this.promotionCoverImageName
            );
          },
          (err) => {
